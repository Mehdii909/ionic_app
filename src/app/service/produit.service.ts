...
export class ProduitService {
  ...
  addProduit(produit: { nom: string; prixUnitaire: number; quantite: number }) {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
    return this.http.post<Produit>(environment.url + '/produits', produit, {
      headers,
    });
  }
  ...
  updateProduit(id: number, produit: Produit) {
    ...
    return this.http.put<Produit>(
      environment.url + '/produits/' + id, produit, { headers }
    );
  }
