...
import {
  ...
  ModalController,
  NavController,
} from '@ionic/angular'; // Importer ModalController
...
export class ProduitComponent implements OnInit {
  ...
  async openEditDialog1(id: number): Promise<void> {
    ...
    editProduitModal.onDidDismiss().then((result) => {
      if (result.data) {
        this.produitService
          .updateProduit(result.data.id, result.data)
          .subscribe(() => {
            this.loadProduits();
          });
      }
      ...
      <ion-list>
        ...
        <ion-item>
          <ion-label
            ...
            {{ produit.prixUnitaire }}</ion-label
          >
        </ion-item>
        <ion-item>
          <ion-label
            ><strong>Quantité :</strong> {{ produit.quantite }}</ion-label
          >
        </ion-item>
      </ion-list>
    </ion-content>
...
//         </ion-buttons>
//         <ion-buttons slot="end">
//           <ion-button [color]="data.confirmColor" (click)="dismiss(true)">{{
//             data.confirmText
//           }}</ion-button>
//         </ion-buttons>
//       </ion-toolbar>
